  ...
  try {
    ...
    if (path.startsWith('results')) {
      ...
      if (req.method === "POST") {
        ...
        if (id === 'bulk') {
          const payload = Array.isArray(req.body?.results) ? req.body.results : [];
          const { data, error } = await supabase.from("exam_results").upsert(payload).select();
          if (error) throw error;
          return res.json(data);
        }
      ...
      if (req.method === "PATCH" && id) {
        if (user.role !== "headteacher" && user.role !== "teacher") return res.status(403).json({ error: "Forbidden" });
        const { data, error } = await supabase.from("exam_results").update(req.body).eq("id", id).select();
        if (error) throw error;
        return res.json(data);
      }
    }
